...
xml.feed "xmlns" => "http://www.w3.org/2005/Atom" do
	...
	blog.articles[0..5].each do |article|
		xml.entry do
			...
			xml.author { xml.name "Andrew Pilsch" }
			xml.content ((if article.data.has_key? "description" then Haml::Filters::Markdown.render(article.data["description"]) else article.summary end) + link_to( "[...]", "#{config[:site_deploy_root]}#{article.url}")), "type" => "html"
		end
	end
end