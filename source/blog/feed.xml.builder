...
xml.feed "xmlns" => "http://www.w3.org/2005/Atom" do
	...
	blog.articles[0..5].each do |article|
		xml.entry do
			...
			xml.author { xml.name "Andrew Pilsch" }
			xml.summary ((if article.data.has_key? "description" then article.data["description"] else article.summary end) + link_to( "[...]", "http://andrew.pilsch.com#{article.url}")), "type" => "html"
